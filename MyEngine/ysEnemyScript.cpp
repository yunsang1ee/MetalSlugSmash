...
void ys::EnemyScript::OnCollisionEnter(Collider* other)
{
	...
	if (!death)
	{
		...
		if (other->GetOwner()->GetLayerType() == enums::LayerType::PlayerLowerBody)
		{
			...
			{
				if (an->GetActive()->getName() != L"°Ô_attack_ÁÂ_±âº»1" && an->GetActive()->getName() != L"°Ô_attack_ÁÂ_±âº»2")
				{
					an->PlayAnimation(L"°Ô_attack_ÁÂ_±âº»1", false);
				...
				if (an->GetActive()->getName() != L"°Ô_attack_±âº»1" && an->GetActive()->getName() != L"°Ô_attack_±âº»2")
				{
					an->PlayAnimation(L"°Ô_attack_±âº»1", false);
					enemyState = EnemyState::Attack;
				}
				moveRight = false;
			}
...
void ys::EnemyScript::OnCollisionStay(Collider* other)
{
	...
	if (!death)
	{
		if (other->GetOwner()->GetLayerType() == enums::LayerType::PlayerLowerBody)
		{
			auto playerLower = other->GetOwner();
			auto an = GetOwner()->GetComponent<Animator>();
			if (an->GetActive()->getName() == L"°Ô_attack_±âº»2"||
				 an->GetActive()->getName() == L"°Ô_attack_ÁÂ_±âº»2")
			{
...
void ys::EnemyScript::NexTAnimation()
{
	...
	if (nowName ==L"°Ô_attack_±âº»1"|| nowName == L"°Ô_attack_ÁÂ_±âº»1")
	{
		if (moveRight)
		{
		...
		{
			an->PlayAnimation(L"°Ô_attack_ÁÂ_±âº»2", false);
		}
		
		return;
	}
	else if (nowName == L"°Ô_attack_±âº»2"|| nowName == L"°Ô_attack_ÁÂ_±âº»2")
	{
...
void ys::EnemyScript::IsAdd()
{
	...
	an->PlayAnimation(L"°Ô_attack_ÁÂ_±âº»1", true);
}
